...
void test_dict_find_if() {
  ...
  assert(found && (int)(intptr_t)found == 0x33);
...
void test_dict_large_data_operations() {
  ...
  for (int i = 0; i < num_items; i++) {
    int result = uu_dict_insert(d, i, (void*)(intptr_t)(i + 100));
    assert(result);
  }
  ...
  // 验证前100个键值对都正确存储
  for (int i = 0; i < 100; i++) {
    assert((void*)(intptr_t)(i + 100) == uu_dict_at(d, i));
  }
  
  // 删除一些数据（每10个删除一个）
  for (int i = 0; i < num_items; i += 10) {
    void* result = uu_dict_remove(d, i);
    assert((void*)(intptr_t)(i + 100) == result);
  }
...
void test_dict_duplicate_keys() {
  ...
  // 尝试插入多个不同的键
  for (int i = 3; i < 10; i++) {
    result = uu_dict_insert(d, i, (void*)(intptr_t)(i + 100));
  ...
  // 值仍应为原始值
  assert((void*)(intptr_t)(5 + 100) == uu_dict_at(d, 5));
...
void test_dict_memory_operations() {
  ...
  for (int i = 0; i < 3; i++) {  // 减少循环次数进一步
    ...
    // 插入少量数据
    for (int j = 0; j < 10; j++) {
      int insert_result = uu_dict_insert(d, j, (void*)(intptr_t)(j + 1000));
      assert(insert_result);
    }
    
    // 验证数据
    for (int j = 0; j < 10; j++) {
      assert((void*)(intptr_t)(j + 1000) == uu_dict_at(d, j));
    }
...
void test_dict_error_usage() {
  ...
  for (int i = 0; i < 100; i++) {
    assert((void*)(intptr_t)i == uu_dict_at(d, i));
  }
  
  for (int i = 0; i < 100; i++) {
    assert((void*)(intptr_t)i == uu_dict_remove(d, i));
  }
